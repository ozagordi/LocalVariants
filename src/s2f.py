    ...
    for b in dna_code:
        for i in range(max_homo + 1, 2, -1):
            for m in re.finditer(b * i, read):
                a, e = m.start(), m.end()
                if refseq[a] == '-' and a not in ins and a > start\
                    and refseq[a + 1:a + i] == ''.join(b * (i - 1)):  # read has an insertion
                    ins[a] = i
                if read[a - 1] == '-' and a not in dels\
                    and a > start and read[a] == refseq[a - 1]:  # read has a deletion
                    dels[a] = i
